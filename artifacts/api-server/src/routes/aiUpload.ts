...
// Uses ONLY confirmed (non-needsReview) transactions for the balance check.
function reconcileBalances(
  ...
  if (openingBalance == null || closingBalance == null) {
    return { ok: true, note: "Saldosid pole — täpsusmõõtmist ei teostatud." };
  }
  const confirmed = transactions.filter((t) => !t.needsReview && t.amount > 0);
  ...
  const totalExpense = confirmed
    ...
    .reduce((s, t) => s + t.amount, 0);
  const computed = openingBalance + totalIncome - totalExpense;
  const diff = Math.abs(computed - closingBalance);

  if (diff <= 0.02) {
    return {
      ok: true,
      note: `Saldod täpsed: ${openingBalance.toFixed(2)} + ${totalIncome.toFixed(2)} − ${totalExpense.toFixed(2)} = ${computed.toFixed(2)} ≈ ${closingBalance.toFixed(2)} EUR`,
    };
  }
  return {
    ok: false,
    note: `Saldo lahknevus: arvutatud ${computed.toFixed(2)}, tegelik ${closingBalance.toFixed(2)}, vahe ${diff.toFixed(2)} EUR`,
  };
}
...
// ── Determine three-state validation status ───────────────────────────────────
...
export function computeValidationStatus(
  ...
  // Any ambiguous transaction → review_required
  if (transactions.some((t) => t.needsReview)) return "review_required";

  const reasons: string[] = [];
  let controlPassCount = 0;

  // Check printed income total
  if (doc.printedIncomeTotal != null) {
    const diff = Math.abs(calculatedIncomeTotal - doc.printedIncomeTotal);
    if (diff > TOLERANCE) {
      reasons.push(
        `Tulude lahknevus: väljavõte ${doc.printedIncomeTotal.toFixed(2)}, ` +
          `arvutatud ${calculatedIncomeTotal.toFixed(2)} (vahe ${diff.toFixed(2)} EUR)`,
      );
    } else {
      controlPassCount++;
    }
  ...
  // Check printed expense total
  if (doc.printedExpenseTotal != null) {
    const diff = Math.abs(calculatedExpenseTotal - doc.printedExpenseTotal);
    if (diff > TOLERANCE) {
      reasons.push(
        `Kulude lahknevus: väljavõte ${doc.printedExpenseTotal.toFixed(2)}, ` +
          `arvutatud ${calculatedExpenseTotal.toFixed(2)} (vahe ${diff.toFixed(2)} EUR)`,
      );
    } else {
      controlPassCount++;
    }
  ...
  // Check opening/closing balance reconciliation
  if (doc.openingBalance != null && doc.closingBalance != null) {
    const computed =
      doc.openingBalance + calculatedIncomeTotal - calculatedExpenseTotal;
    const diff = Math.abs(computed - doc.closingBalance);
    if (diff > TOLERANCE) {
      reasons.push(
        `Saldo lahknevus: arvutatud ${computed.toFixed(2)}, ` +
          `lõppsaldo ${doc.closingBalance.toFixed(2)} (vahe ${diff.toFixed(2)} EUR)`,
      );
    } else {
      controlPassCount++;
    }
  ...
  return "unverified"; // no control data available — not invalid, just unconfirmed
}
...
//
// Readable  + not bank  → return plain text to AI                (no API call)
// Readable  + bank      → extractBankStatementDirect()           (1 API call)
// Garbled   (any type)  → extractScannedPdf() for actual content (1 API call)
//                         └─ if result looks like a bank statement
//                            → extractBankStatementDirect()      (2nd API call)
//                            else → return scanned text to AI
//
// This ensures generic scanned documents (resumes, invoices, contracts, etc.)
// always receive a general-purpose read pass, never a bank-specific prompt.

async function extractPdf(
  buffer: Buffer,
  filename: string,
): Promise<PdfExtractResult> {
  // Step 1: Try normal machine-readable PDF text extraction.
  try {
    const text = await extractPdfText(buffer);

    if (!isGarbledText(text)) {
      // Generic readable PDF:
      // school files, invoices, contracts, statements, reports, etc.
      // Chat upload never routes into the Money/bank-import pipeline.
      return {
        text,
        usedOCR: false,
      };
    ...
    // extractPdfText() never throws PDF_NO_TEXT — an empty/garbled result is
    // returned as a string and detected by isGarbledText(), causing fall-through.
    // Any error here is an unexpected failure (corrupt PDF, pdf-parse crash, etc.)
    // — log it and fall through to the OCR path.
    console.error("[pdf] text extraction error:", err instanceof Error ? err.message : err);
  }

  // Step 2: Garbled / scanned / image-only PDF.
  // Use generic OCR only — never bank-specific extraction from AI chat.
  console.log(`[pdf] unreadable text → generic OCR: ${filename}`);
...
router.post("/ai/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    if (isPdf) {
      ...
      try {
        result = await extractPdf(file.buffer, file.originalname);
      } catch (err) {
        const msg = err instanceof Error ? err.message : "";
        // PDF_NO_TEXT       → document has genuinely no readable/extractable content
        // PDF_EMPTY_MODEL_OUTPUT → vision model call succeeded but returned nothing
        // Both are non-retryable; send distinct 422 codes so the client can
        // display an appropriate message.
        if (msg === "PDF_NO_TEXT" || msg === "PDF_EMPTY_MODEL_OUTPUT") {
      ...
      // Defensive guard: placeholder strings from old extraction paths are not real content.
      const PLACEHOLDER_RE =
        /^Content extracted from .+ via (?:direct PDF|image) analysis\.$/;
      if (
        !result.transactions?.length &&
        PLACEHOLDER_RE.test(result.text.trim())
      ) {
        console.error(
          `[pdf] placeholder text detected as content — treating as extraction failure: ${file.originalname}`,
        );
      ...
      res.json({
        content: result.text.slice(0, 30_000),
      ...
      // Non-PDF: standard extraction
      const text = await extractText(
        file.buffer,
        file.originalname,
        file.mimetype,
      );
      // Cap at 30 000 chars to stay within token budgets
      res.json({
...
//
//   - Accepts CSV (.csv) and Excel (.xlsx / .xls) only.
//   - PDF is NOT accepted here.  Generic AI PDF upload remains at /api/ai/upload.
//   - Parsing is fully deterministic — no OpenAI, no OCR, no AI of any kind.
//   - Returns { transactions, bankMeta } on success.
//   - Returns HTTP 422 { error: "UNSUPPORTED_BANK_FILE_FORMAT" | "NO_TRANSACTIONS_FOUND" }
...
router.post("/ai/bank-import", upload.single("file"), async (req, res) => {
  ...
  if (!isXlsx && !isXls && !isCsv) {
    res.status(400).json({
      error: "Only CSV and Excel files (.csv, .xlsx, .xls) are supported for bank statement import.",
    });
    return;
  }
  ...
  try {
    ...
    // This is application code — never inferred from descriptions.
    const rawTransactions: BankTransaction[] = parsed.transactions.map((row) => {
      ...
      const direction: "income" | "expense" = credit !== null ? "income" : "expense";
    ...
    // If opening/closing balance was detected in the file, the running-balance
    // chain is validated deterministically (same as the structural PDF path).
    // If not, validationStatus = "unverified" — import is still allowed.
    // Printed income/expense totals are null for CSV (rarely printed in files).
    ...
    const bankMeta: BankMeta = {
      ...
      openingBalance: parsed.controls.openingBalance ?? undefined,
      closingBalance: parsed.controls.closingBalance ?? undefined,
      pagesTotal: 1,
      pagesProcessed: 1,
      ...
      validationErrors: [...parsed.warnings, ...post.validationErrors],
    ...
    const message = err instanceof Error ? err.message : "File could not be processed.";
    console.error(`[BANK IMPORT CSV] Unexpected error for ${file.originalname}:`, message);
    res.status(422).json({ error: message });
  }
});
