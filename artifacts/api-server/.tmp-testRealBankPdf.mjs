...
// src/lib/testRealBankPdf.ts
async function run() {
  const pdfPath = path.resolve(process.cwd(), "src/lib/statement.pdf");
  if (!fs.existsSync(pdfPath)) {
    console.error(`statement.pdf not found at: ${pdfPath}`);
    process.exit(1);
  }
  ...
  console.log({
    positionalItems: items.length,
    visualRows: result.rows.length,
