...
export default function HabitsPage() {
  ...
  const recommendation = useMemo(() => {
    ...
    const weekFilledText = t('habits.recommend.weekFilled', lang)
      .replace('{done}', String(weekDone))
      .replace('{total}', String(Math.max(weekTotal, 1)))
    return {
      habitId: target?.id ?? null,
      habitTitle: target?.title ?? (lang === 'en' ? 'Workout' : 'Treeni'),
      summary: lang === 'en'
        ? 'This habit needs a bit more attention this week.'
        : 'Trenn vajab sel nädalal veidi rohkem tähelepanu.',
      reason: weekFilledText,
      tips: lang === 'en' ? [
        'Add a reminder for this habit.',
        'Plan your workout at a fixed time.',
        'Start with a smaller goal to stay consistent.',
      ] : [
    ...
    <div className="flex flex-col lg:flex-row gap-6 p-6 max-w-[1400px] mx-auto w-full">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold text-[#1A1F36]">{t('habits.title', lang)}</h1>
            <p className="text-sm text-[#64748B] mt-0.5">
              {t('habits.subtitle', lang).replace('{n}', String(habits.length)).replace('{active}', String(activeCount))}
            </p>
          </div>
          <button
        ...
        <div className="flex items-center gap-1 p-1 bg-white rounded-xl border border-[#ECECF2] w-fit">
          {([
            { key: 'all',       label: t('habits.filter.all',    lang).replace('{n}', String(habits.length))     },
            { key: 'active',    label: t('habits.filter.active', lang).replace('{active}', String(activeCount))  },
            { key: 'paused',    label: t('habits.filter.paused', lang).replace('{n}', String(pausedCount))       },
            { key: 'completed', label: t('habits.filter.done',   lang).replace('{n}', String(completedCount))    },
        ...
        <div className="bg-white rounded-2xl border border-[#ECECF2] overflow-hidden">
                ...
                <div className="flex-shrink-0 w-24 text-center">
                  {habit.status === 'paused' ? (
                    <>
                      ...
                      <p className="text-xs text-[#94A3B8]">{t('habits.streak.paused', lang)}</p>
                    </>
                  ) : (
                    <>
                      <p className="text-sm font-bold text-[#1A1F36]">{habit.streak}</p>
                      <p className="text-xs text-[#94A3B8]">{t('habits.streak.days', lang)}</p>
                    </>
                  )}
                </div>
                ...
                <div className="relative flex-shrink-0" ref={(el) => { menuRefs.current[habit.id] = el }}>
                  ...
                  {menuOpenId === habit.id && (
                    <div className={`absolute right-0 ${menuUp ? 'bottom-full mb-1' : 'top-full mt-1'} w-48 bg-white rounded-xl border border-[#ECECF2] shadow-lg z-20 py-1`}>
                      ...
                      >
                        {isDoneToday ? <><X size={14} /> {t('habits.menu.cancelToday', lang)}</> : <><Check size={14} /> {t('habits.menu.markDone', lang)}</>}
                      </button>
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                        ...
                        className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#1A1F36] hover:bg-[#F8F7F4] transition-colors"
                      >
                        <Pencil size={14} /> {t('habits.menu.edit', lang)}
                      </button>
                      {habit.status === 'paused' ? (
                        <button
                          onClick={(e) => {
                          ...
                          className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#1A1F36] hover:bg-[#F8F7F4] transition-colors"
                        >
                          <Play size={14} /> {t('habits.menu.resume', lang)}
                        </button>
                      ) : (
                        <button
                          onClick={(e) => {
                          ...
                          className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#1A1F36] hover:bg-[#F8F7F4] transition-colors"
                        >
                          <Pause size={14} /> {t('habits.menu.pause', lang)}
                        </button>
                      )}
                      <button
                        onClick={(e) => {
                      ...
                      >
                        <Trash2 size={14} /> {t('habits.menu.delete', lang)}
                      </button>
                    </div>
                  )}
                </div>
          ...
          {displayed.length === 0 && (
            <div className="py-12 text-center">
              <p className="text-sm font-medium text-[#1A1F36]">{t('habits.empty.title', lang)}</p>
              <p className="text-xs text-[#94A3B8] mt-1">{t('habits.empty.body', lang)}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <aside className="w-full lg:w-80 flex-shrink-0 flex flex-col gap-4">
        ...
        {/* Ülevaade */}
        <div className="bg-white rounded-2xl border border-[#ECECF2] p-5">
          <h3 className="text-sm font-semibold text-[#1A1F36] mb-4">{t('habits.overview.title', lang)}</h3>
          <div className="flex items-center gap-4">
            ...
            <div className="relative w-20 h-20 flex-shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[9px] text-[#94A3B8] -mt-0.5">{t('habits.successRate', lang)}</span>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-2">
              {[
                { label: t('habits.quality.excellent', lang), sub: '(80%+)',  color: '#4ADE80', count: suurepärane },
                { label: t('habits.quality.good',      lang), sub: '(50-79%)', color: '#3B82F6', count: hea },
                { label: t('habits.quality.needsWork', lang), sub: '(<50%)',  color: '#FDE68A', count: vajab },
              ].map((row) => (
              ...
              <div className="pt-1 border-t border-[#F4F4F0]">
                <p className="text-xs font-semibold text-[#1A1F36]">{t('habits.thisWeek', lang)}: {weekDone}/{weekTotal}</p>
                <p className="text-[10px] text-[#94A3B8]">{t('habits.allAvg', lang)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {longestStreak && longestStreak.streak > 0 && (
          <div className="bg-white rounded-2xl border border-[#ECECF2] p-5">
            <h3 className="text-sm font-semibold text-[#1A1F36] mb-4">{t('habits.streak.title', lang)}</h3>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-lg font-bold text-[#1A1F36]">{longestStreak.streak} {t('habits.streak.days', lang)}</p>
        ...
        <div className="bg-white rounded-2xl border border-[#ECECF2] p-5">
          <h3 className="text-sm font-semibold text-[#1A1F36] mb-4">{t('habits.breakdown.title', lang)}</h3>
          <div className="flex flex-col gap-2">
            {[
              { label: t('habits.breakdown.active', lang), count: activeCount,    color: '#4ADE80' },
              { label: t('habits.breakdown.paused', lang), count: pausedCount,    color: '#FDE68A' },
              { label: t('habits.breakdown.done',   lang), count: completedCount, color: '#CBD5E1' },
            ].map((row) => (
        ...
        <div className="bg-gradient-to-br from-[#6F5AE8] to-[#7C6BF0] rounded-2xl p-5 text-white">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold">{t('habits.ai.title', lang)}</h3>
          </div>
          <p className="text-sm leading-relaxed text-white/90 mb-4">
            {t('habits.ai.body', lang)}
          </p>
          <button
          ...
          >
            {t('habits.recommend.openHabit', lang)}
          </button>
        </div>
      </aside>
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#F4F4F0] sticky top-0 bg-white">
              <h2 className="text-base font-semibold text-[#1A1F36]">
                {editingId ? t('habits.modal.editTitle', lang) : t('habits.modal.addTitle', lang)}
              </h2>
              <button
                onClick={handleCancelForm}
            ...
            <div className="px-5 py-4 flex flex-col gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">
                  {t('habits.modal.nameLabel', lang)} <span className="text-[#E11D48]">*</span>
                </label>
                <input
                  type="text"
                  value={form.title}
              ...
              {/* Description */}
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">{t('habits.modal.descLabel', lang)}</label>
                <input
                  type="text"
              ...
              {/* Category */}
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">{t('habits.modal.categoryLabel', lang)}</label>
              ...
              {/* Icon */}
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">{t('habits.modal.iconLabel', lang)}</label>
                <div className="flex flex-wrap gap-2">
              ...
              {/* Color */}
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">{t('habits.modal.colorLabel', lang)}</label>
                <div className="flex flex-wrap gap-2">
                  {COLOR_OPTIONS.map((c) => (
                    <button
                      key={c.color}
              ...
              {/* Goal per day */}
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">{t('habits.modal.goalLabel', lang)}</label>
                <input
                  type="number"
                  min={1}
              ...
              {/* Recurrence */}
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">{t('habits.modal.recurrenceLabel', lang)}</label>
                <div className="flex flex-wrap gap-2">
                  {([
                    { key: 'daily',    label: t('habits.modal.daily', lang) },
                    { key: 'weekdays', label: t('habits.modal.weekdays', lang) },
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#F4F4F0] sticky top-0 bg-white">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-semibold text-[#1A1F36]">{t('habits.manage.title', lang)}</h2>
              </div>
              <button
                onClick={() => setManageOpen(false)}
                className="w-8 h-8 rounded-lg flex items-center justify-center text-[#94A3B8] hover:bg-[#F8F7F4] hover:text-[#1A1F36] transition-colors"
            ...
            <div className="px-5 py-4 flex flex-col gap-2">
              {habits.map((habit) => (
                <div key={habit.id} className="flex items-center gap-3 p-3 rounded-xl border border-[#ECECF2] hover:bg-[#FAFAF8] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[#1A1F36] truncate">{habit.title}</p>
                    <p className="text-xs text-[#94A3B8]">{STATUS_LABEL_LANG[habit.status]}</p>
                  </div>
              ...
              {habits.length === 0 && (
                <p className="text-sm text-[#94A3B8] py-6 text-center">{t('habits.manage.empty', lang)}</p>
              )}
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            <div className="px-5 py-5 flex flex-col items-center text-center">
              ...
              <h3 className="text-base font-semibold text-[#1A1F36] mb-1">{t('habits.deleteConfirm.title', lang)}</h3>
              <p className="text-sm text-[#64748B]">{t('habits.deleteConfirm.body', lang)}</p>
            </div>
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#F4F4F0]">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-semibold text-[#1A1F36]">{t('habits.recommend.title', lang)}</h2>
              </div>
              <button
                onClick={() => setRecommendOpen(false)}
                className="w-8 h-8 rounded-lg flex items-center justify-center text-[#94A3B8] hover:bg-[#F8F7F4] hover:text-[#1A1F36] transition-colors"
            ...
            <div className="px-5 py-4 flex flex-col gap-4">
              <div>
                <p className="text-xs font-medium text-[#94A3B8] uppercase tracking-wide mb-1">{t('habits.title', lang)}</p>
              ...
              <div>
                <p className="text-xs font-medium text-[#94A3B8] uppercase tracking-wide mb-1">{t('habits.recommend.reason', lang)}</p>
                <p className="text-sm text-[#374151] leading-relaxed">{recommendation.reason}</p>
              </div>

              <div>
                <p className="text-xs font-medium text-[#94A3B8] uppercase tracking-wide mb-2">{t('habits.recommend.tips', lang)}</p>
