...
export function getLocalAppearance(): AppearanceSettings {
  return readLocal()
}

// --- Pub/sub: subscribers are notified after every applyAppearance call ---
type AppearanceListener = (settings: AppearanceSettings) => void
const _listeners = new Set<AppearanceListener>()

export function subscribeToAppearance(cb: AppearanceListener): () => void {
  _listeners.add(cb)
  return () => _listeners.delete(cb)
}

// --- System theme listener (active only when themeMode === 'system') ---
let _sysMQ: MediaQueryList | null = null
let _sysMQHandler: (() => void) | null = null

function setupSystemListener(mode: ThemeMode): void {
  if (_sysMQ && _sysMQHandler) {
    _sysMQ.removeEventListener('change', _sysMQHandler)
    _sysMQHandler = null
  }
  if (mode === 'system') {
    ...
    _sysMQHandler = () => {
      const s = readLocal()
      if (s.themeMode === 'system') applyAppearance(s)
    }
    _sysMQ.addEventListener('change', _sysMQHandler)
  }
}
...
export function applyAppearance(settings: AppearanceSettings): void {
  ...
  writeLocal(settings)
  setupSystemListener(settings.themeMode)
