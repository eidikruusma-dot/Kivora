/**
 * School change #9 — Kontrolltööd's active list previously showed every
 * exam of type "kontrolltöö", with completed ones (status === "tehtud")
 * simply sorted to the bottom with a "Tehtud" badge. ExamsTab now splits
 * the same `exams` array it always received into:
 *   - active: status === "ootel" only — the main list, keeping its
 *     existing daysLeft sort;
 *   - History: status === "tehtud" only, grouped by subject the same
 *     alphabetical/color-accent approach School Tasks History uses
 *     (School change #7), reimplemented here as a small separate
 *     `groupExamsBySubjectAlpha` (using Exam's own iconColor/iconBg field
 *     names) rather than reusing that function directly, so School Tasks
 *     History stays untouched.
 *
 * Entirely derived from the existing `status` field (already exercised by
 * School change #8's mark-done/undone action) — no new field, collection,
 * or migration. Because active/completed partition `exams` by mutually
 * exclusive status values, a given exam can never appear in both.
 *
 * Rendering the same row markup twice (active list and History) without
 * duplicating it required pulling the existing inline per-exam-row JSX out
 * into a standalone `ExamRow` component, and the per-subject-group heading
 * into `SubjectExamGroups` — both local to SchoolPage.tsx and used only by
 * ExamsTab. Eksamid, School Tasks History, Calendar sync, and School<->Tasks
 * sync are untouched.
 *
 * No React rendering harness is available in this repo for SchoolPage.tsx
 * (see schoolTasksTabHistorySection.test.ts for the closely analogous
 ...
 *   - the UI wiring (History heading, reused ExamRow/color logic, existing
 *     actions untouched) is proven structurally.
 *
 * Compile and run standalone:
 *   cd artifacts/planner-app
 *   npx vitest run src/lib/__tests__/schoolExamHistorySection.test.ts
 */
...
function examsTabSource(): string {
  const match = SCHOOL_PAGE_SRC.match(/interface SubjectExamGroup \{[\s\S]*?\nfunction ExamsTab\([\s\S]*?\n}\n/)
  expect(match).not.toBeNull()
  return match![0]
}

// ── The active/History split + grouping, reproduced verbatim from
// ExamsTab so its correctness can be exercised directly. ───────────────────
...
describe('the active list shows only status === "ootel", never a completed test', () => {
...
describe('status === "tehtud" tests appear in History, grouped by subject', () => {
...
describe('subject colors are reused in Exam History (existing iconColor/iconBg, no new color system)', () => {
...
// ── done -> History, undone -> active ───────────────────────────────────────

describe('marking a test done moves it to History; marking it undone returns it to active immediately', () => {
  it('re-splitting after status flips to "tehtud" relocates the exam into History', () => {
    ...
    // Same exam, marked done — this is exactly what ExamDetailModal's
    // onMarkDone (School change #8) does to `status`.
...
describe('ExamsTab implements this exact active/History split and grouping (structural)', () => {
  const src = examsTabSource()
  ...
  it('History groups completed exams by subject via groupExamsBySubjectAlpha, alphabetically, with the group color from the first exam\'s existing iconColor/iconBg', () => {
    expect(src).toMatch(
      /Array\.from\(new Set\(list\.map\(\(e\) => e\.subject\)\)\)\.sort\(\s*\n?\s*\(a, b\) => a\.localeCompare\(b, "et"\),?\s*\n?\s*\)/,
    )
    expect(src).toMatch(/list\.filter\(\(e\) => e\.subject === subject\)/)
    expect(src).toMatch(/color:\s*first\.iconColor/)
    expect(src).toMatch(/bg:\s*first\.iconBg/)
  ...
  it('does not call School Tasks History\'s own grouping function or reference Eksamid\'s components — it uses its own groupExamsBySubjectAlpha', () => {
    expect(src).not.toMatch(/= groupTasksBySubjectAlpha\(/)
    expect(src).not.toMatch(/<EksamidTab/)
    expect(src).not.toMatch(/<EksamDetailModal/)
  })

  it('School Tasks History\'s own grouping function is untouched (still defined once, still called only from TasksTab)', () => {
    const defCount = (SCHOOL_PAGE_SRC.match(/function groupTasksBySubjectAlpha\(/g) ?? []).length
...
describe('existing Kontrolltöö detail/edit/delete/link actions are preserved — every row (active or History) opens the same ExamDetailModal', () => {
  const src = examsTabSource()

  it('both the active list and History rows render via the same ExamRow component (no divergent row implementation)', () => {
    expect(src).toMatch(/<ExamRow[\s\S]*?onExamClick=\{onExamClick\}/)
    const subjectExamGroupsBlock = src.match(/function SubjectExamGroups[\s\S]*?\n}\n/)?.[0] ?? ''
    expect(subjectExamGroupsBlock).toMatch(/<ExamRow/)
  })

  it('ExamRow still opens the detail modal via onExamClick(exam), unchanged', () => {
    const examRowBlock = src.match(/function ExamRow[\s\S]*?\n}\n/)?.[0] ?? ''
    expect(examRowBlock).toMatch(/onClick=\{\(\) => onExamClick\(exam\)\}/)
  })

  it('ExamsTab still receives and forwards onEdit/onDelete props (edit/delete/link stay reachable via the detail modal, unaffected by the split)', () => {
