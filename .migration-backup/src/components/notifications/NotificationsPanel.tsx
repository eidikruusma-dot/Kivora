      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-xl border border-[#E8E6E0] shadow-lg z-50 overflow-hidden">
          <div className="px-4 py-3 border-b border-[#F0F0F0] flex items-center justify-between">
            <h3 className="text-sm font-bold text-[#1A1F36]">Teavitused</h3>
            {unreadCount > 0 && (
          ...
          <div className="max-h-[320px] overflow-y-auto">
            {notifications.length === 0 ? (
              <p className="px-4 py-8 text-center text-sm text-[#94A3B8]">Uusi teavitusi pole.</p>
            ) : (
              notifications.map((n) => {
          ...
          >
            Vaata kõiki teavitusi <ArrowRight size={12} />
          </button>
        </div>
      )}
    </div>
