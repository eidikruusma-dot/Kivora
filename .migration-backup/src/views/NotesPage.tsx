...
export default function NotesPage() {
  ...
  const handleSave = () => {
    if (!form.title.trim()) {
      setFormError('Pealkiri on kohustuslik.')
      return
    }
    if (!form.content.trim()) {
      setFormError('Sisu on kohustuslik.')
      return
    }
    if (editingId) {
    ...
    <div className="flex flex-col lg:flex-row gap-6 p-6 max-w-[1400px] mx-auto w-full">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold text-[#1A1F36]">Märkmed</h1>
            <p className="text-sm text-[#64748B] mt-0.5">{notes.length} märget · {FOLDER_LIST.length} kausta</p>
          </div>
          <button
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1 relative">
                    <button
                      ...
                      title={note.starred ? 'Eemalda oluline' : 'Märgi oluliseks'}
                    >
                      <Star
                        size={14}
                        className={
                    ...
                    <div ref={(el) => { menuRefs.current[note.id] = el }}>
                      ...
                      {menuOpenId === note.id && moveMenuId !== note.id && (
                        <div className="absolute right-0 top-full mt-1 w-40 bg-white rounded-xl border border-[#ECECF2] shadow-lg z-20 py-1">
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#1A1F36] hover:bg-[#F8F7F4] transition-colors"
                          >
                            <FolderInput size={14} /> Teisalda kausta
                          </button>
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                      ...
                      {/* Move submenu */}
                      {moveMenuId === note.id && (
                        <div className="absolute right-0 top-full mt-1 w-40 bg-white rounded-xl border border-[#ECECF2] shadow-lg z-30 py-1">
                          ...
                          {FOLDER_OPTIONS.map((f) => (
                            ...
                            >
                              ...
                              {f}
                              {f === note.folder && <span className="ml-auto text-[10px]">praegune</span>}
                            </button>
                          ))}
                        </div>
                      )}
        ...
        {filteredNotes.length === 0 && (
          <div className="bg-white rounded-2xl border border-[#ECECF2] flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm font-medium text-[#1A1F36]">Märkmeid ei leitud</p>
            <p className="text-xs text-[#94A3B8] mt-1">Proovi teist otsusõna või kausta.</p>
          </div>
        )}
      </div>
      ...
      <aside className="w-full lg:w-80 flex-shrink-0 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl border border-[#ECECF2] p-5">
          <h3 className="text-sm font-semibold text-[#1A1F36] mb-4">Märkmete ülevaade</h3>
          <div className="flex items-center gap-4">
            <div className="relative w-20 h-20 flex-shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[10px] text-[#94A3B8] -mt-0.5">märget</span>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-[#ECECF2] p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-[#1A1F36]">Kaustad</h3>
            <button
              onClick={() => {}}
        ...
        <div className="bg-gradient-to-br from-[#6F5AE8] to-[#7C6BF0] rounded-2xl p-5 text-white">
          ...
          <p className="text-sm leading-relaxed text-white/90">
            Sul on 5 isiklikku märget, mida pole viimase nädala jooksul uuendatud. Soovitan need üle vaadata ja vanad ideed arhiivida.
          </p>
        </div>
      </aside>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#F4F4F0]">
              <h2 className="text-base font-semibold text-[#1A1F36]">
                {editingId ? 'Muuda märget' : 'Uus märge'}
              </h2>
              <button
                onClick={handleCancelForm}
            ...
            <div className="px-5 py-4 flex flex-col gap-4">
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">
                  Pealkiri <span className="text-[#E11D48]">*</span>
                </label>
                <input
                  type="text"
                  value={form.title}
              ...
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">
                  Sisu <span className="text-[#E11D48]">*</span>
                </label>
                <textarea
                  value={form.content}
                  onChange={(e) => {
              ...
              >
                  ...
                  />
                  {form.starred ? 'Märgitud oluliseks' : 'Märgi oluliseks'}
                </button>
      ...
      {detailNote && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#F4F4F0]">
              <h2 className="text-base font-semibold text-[#1A1F36]">Märkme sisu</h2>
              <button
                onClick={() => setDetailNote(null)}
                className="w-8 h-8 rounded-lg flex items-center justify-center text-[#94A3B8] hover:bg-[#F8F7F4] hover:text-[#1A1F36] transition-colors"
              >
        ...
        >
          ...
          >
            <div className="px-5 py-5 flex flex-col items-center text-center">
              ...
              <h3 className="text-base font-semibold text-[#1A1F36] mb-1">Kustuta märge?</h3>
